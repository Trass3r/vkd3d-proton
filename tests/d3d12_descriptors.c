    ...
    for (j = 0; j < TYPE_COUNT; j++)
    {
        for (i = 0; i < TYPE_COUNT; i++)
        {
            ...
            ID3D12Device_CopyDescriptorsSimple(context.device, 1, ID3D12DescriptorHeap_GetCPUDescriptorHandleForHeapStart(gpu_heap),
                    get_cpu_handle(context.device, cpu_heap, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, descriptor_type), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
                    ...
                    descriptor_type == CBV)
            {
                ...
                accept_zero = true;
                    ...
                    (descriptor_type == UAV_TEX && access_type == SRV_TEX))
            {
                working_aliasing = true;
            }
                    ...
                    (access_type == UAV_RAW_BUFFER && descriptor_type == SRV_RAW_BUFFER))
            {
                ...
                working_aliasing = true;
                    ...
                    (access_type == UAV_TYPED_BUFFER && descriptor_type == SRV_TYPED_BUFFER))
            {
                /* Trivial */
                working_aliasing = true;
            }
            ...
            if (i == j || working_aliasing)
            {
                ...
                if (access_type == CBV)
                    expected += 16.0f;
                todo_if(i != j) ok(value == expected || (accept_zero && value == 0.0f), "Descriptor type %u, Shader type %u, expected %f, got %f.\n", descriptor_type, access_type, expected, value);
            }
            else if (value != expected)
                skip("Accessing type %u is not compatible with descriptor type %u (as expected).\n", access_type, descriptor_type);
            else
